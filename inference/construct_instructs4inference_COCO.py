...
def construct_instructs_VregVocEc(dim_name, num_examples, aff_name, data_name, Task_prompt, input_path, output_path):
    ...
    test_withretri = torch.load(input_path + "./retrieval/data4RAG/%s/%s/%s/test-Retribasedon-%s.pth" % (data_name, aff_name, dim_name, aff_name) )
...
def construct_instructs_VregVocEc_addexpl(dim_name, num_examples, aff_name, data_name, Task_prompt, input_path, output_path):
    test_withretri = torch.load(
        input_path + "./retrieval/data4RAG/%s/%s/%s/test-Retribasedon-%s.pth" % (data_name, aff_name, dim_name, aff_name))
...
def construct_instructs_EIregEIoc(dim_name, num_examples, aff_name, data_name, Task_prompt, input_path, output_path):
    ...
    test_withretri = torch.load(
        input_path + "./retrieval/data4RAG/%s/%s/%s/test-Retribasedon-%s.pth" % (data_name, aff_name+"/"+emotion_name, dim_name, aff_name+"-"+emotion_name))
    test_datas = []
    test_datas.extend(test_withretri)
...
def construct_instructs_EIregEIoc_addexpl(dim_name, num_examples, aff_name, data_name, Task_prompt, input_path, output_path):
    ...
    test_withretri = torch.load(
        input_path + "./retrieval/data4RAG/%s/%s/%s/test-Retribasedon-%s.pth" % (
...
if __name__ == "__main__":
    ...
    Task_prompt = '''Classify the text regarding COVID-19 conspiracy theories or misinformation into one of the following three classes:
    0. Unrelated.
